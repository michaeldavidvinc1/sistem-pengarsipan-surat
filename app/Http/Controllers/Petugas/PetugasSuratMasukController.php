...
class PetugasSuratMasukController extends Controller
{
        ...
        if ($request->ajax()) {
            ...
            return datatables()::of($query)
                ...
                ->addColumn('action', function ($row) {
                    $btnDisposisi = '<a href="'.route('petugas.disposisi.index').'" class="disposisi btn btn-warning btn-sm">Disposisi</a>';
                    $btnEdit  = '<a href="'.route('petugas.suratMasuk.edit', $row->id).'" class="edit btn btn-primary btn-sm">Edit</a>';
                    return '<div class="d-flex gap-2">'.$btnDisposisi.$btnEdit.'</div>';
                })
                ->make(true);
        }
        return view('petugas.surat_masuk.index');
    }

    public function create(){
        $jenisSurat = JenisSurat::all();
        return view('petugas.surat_masuk.create', compact('jenisSurat'));
    }
    ...
    public function store(Request $request){
        ...
        return redirect()->route('petugas.suratMasuk.index');
    }

    public function edit($id){
        $data = SuratMasuk::findOrFail($id);
        $jenisSurat = JenisSurat::all();
        return view('petugas.surat_masuk.edit', compact('data', 'jenisSurat'));
    }
    ...
    public function previewPdf($id){
        ...
        return response($content)
            ...
            ->header('Content-Disposition', 'inline; filename="' . $suratMasuk->berkas_sm . '.pdf"');
